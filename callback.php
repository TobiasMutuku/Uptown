...
if ($conn->connect_error) {
  die("Connection failed: " . $conn->connect_error);
    ...
    $sql = "SELECT `ResponseCode` FROM `responses` WHERE LOCATE('$date', `date`) AND `phonenumber`='$phonenumber'";
...
$conn->close();
    ...
    $conn = mysqli_connect("$host","$user","$pass","$db"); //Connets to the dashboard
...
//insert statement
$sql = "INSERT INTO responses(MerchantRequestID, CheckoutRequestID, ResponseCode, ResponseDescription , amount, receiptnumber, balance, date, phonenumber)
